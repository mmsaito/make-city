...
structure Frame = struct
  ...
  let
    ...
    fun cup pop = let
      ...
      fun loop pop = let
        ...
        val hmObj = {id = hm, nVis = zeroNVis(), pTrns = zeroPTrns(), size = n, betaN = betaNHome}
  ...
  fun genArea
    ...
    (area_t
    , #1 popHome
    , {sch   = Vector.tabulate (nSch  , fn id => sch  {place_k = Sch, area_t = area_t, id = id})
      ,corp  = Vector.tabulate (nCorp , fn id => corp {place_k = Corp, area_t = area_t, id = id})
      ,super = Vector.tabulate (nSuper, fn id => super{place_k = Super, area_t = area_t, id = id})
      ,park  = Vector.tabulate (nPark , fn id => park {place_k = Park, area_t = area_t, id = id})
      ,home  = #2 popHome
      }
    )
  (*
   mkPlaces [(SCH,3,rule),(CORP,5,rule)] 
   などのIFの方がよくないか
   *)
  *)
